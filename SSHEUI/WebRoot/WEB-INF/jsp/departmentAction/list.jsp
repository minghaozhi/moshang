...
<script type="text/javascript">
	$(function() {
		//定义表格上的工具条
		var toolbar = [ 
			{text : '添加', iconCls : 'icon-add', handler : addDepartment},//添加用户
			{text : '删除', iconCls : 'icon-cancel', handler : deleteDepartment},//删除用户，支持批量删除
			{text : '修改', iconCls : 'icon-edit', handler : editDepartment}//修改用户，只能单个修改
//	 		{text : '导出Excel', iconCls : 'icon-redo', handler : exportDepartment}//初始化用户密码，支持批量操作
		];
		//加载datagrid数据表格
		$('#dategrid').datagrid({
			...
			columns : [ [ 
				...
				{field : 'address', title : '部门地址', width : 80, sortable : true}, 
				...
				{field : 'deptState', title : '部门状态', width : 80, sortable : true},
	...
	//添加 和 修改 用户 共用的弹窗
	function addDepartment(action, id) {
		//默认是添加用户
		var title = "添加用户";
		var url = 'department_addUI.action';
		
		//如果传入的参数 action 是 edit, 则表示要修改用户
	/*	if(action == 'edit'){
			title = "编辑用户";
			url = 'department_editUI.action?id='+id;
		}*/
	...
	function editDepartment(){
		...
		if($checkedRows.length == 0){
			 ...
			 * 所以这里利用addDepartment()来显示修改页面
			 * 传递到后台时，准备回显数据
			 */
			 var editId = $checkedRows[0].id;

 //    		var postData = {'editId': editId};
     		
     		var title = "编辑部门";
    		var url = 'department_editUI.action?id='+editId;
...
//			 addDepartment('edit',$checkedRows[0].id);
		}//end of if
	}//end of editUser
