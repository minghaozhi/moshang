...
public class UserServiceImpl extends BaseServiceImpl<User> implements UserService {
	...
	public String getUserMenu(User user) {
		...
		//json格式里不能用单引号，只能用双引号，前端才能解析。
		
		// 根据登录的用户名loginName，获取该用户对应的顶级菜单
		String topMenuSql = "select a.id,a.name,a.icon,a.url from wxw_sysmenu a where length(id)=2"//
		...
		if (!(resultList == null || resultList.size() == 0)) {
			for (int i = 0; i < resultList.size(); i++) {
				...
				resultStr.append("\"menus\":");
				String subMenuSql = "select a.id,a.name,a.icon,a.url from wxw_sysmenu a "//
